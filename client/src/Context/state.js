...
const ProviderState = ({ children }) => {
  ...
  const getState = async (id) => {
    try {
      const response = await fetch(`http://localhost:8000/state/${id}`, {
        method: 'GET',
        headers: {
  ...
  return (
    <context.Provider value={{ getState,getStates,updateState,updateShow, getTheatre, getShows, updateTheatre, updateMovie, theatreData, setclickid, clickid, setTheatreData, isusersignin, isAdmin, isprofilename, list, user, Movie, addMovie, deleteItem, getMovie, movieData, id, setId, loading, AddTheatre, updateUser, deleteTheatre, addShows }}>
      {children}
    </context.Provider>
  )
}
